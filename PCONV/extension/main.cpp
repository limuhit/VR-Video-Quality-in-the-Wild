...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    py::class_<channel_group_param_opt>(m,"ChannelGroupParamOp")
        .def(py::init<int, int, int, bool, int, bool>())
        .def("to", &channel_group_param_opt::to)
        .def("forward", &channel_group_param_opt::forward_cuda)
        .def("backward", &channel_group_param_opt::backward_cuda)
        .def("forward_uneven", &channel_group_param_opt::forward_cuda_uneven)
        .def("backward_uneven", &channel_group_param_opt::backward_cuda_uneven);

    py::class_<channel_group_reshape_opt>(m,"ChannelGroupReshapeOp")
        .def(py::init<int, int, int, bool>())
        .def("to", &channel_group_reshape_opt::to)
        .def("forward", &channel_group_reshape_opt::forward_cuda)
        .def("backward", &channel_group_reshape_opt::backward_cuda)
        .def("forward_uneven", &channel_group_reshape_opt::forward_cuda_uneven)
        .def("backward_uneven", &channel_group_reshape_opt::backward_cuda_uneven);

    py::class_<rotate_opt>(m,"RotateOp")
        .def(py::init<int, int, bool>())
        .def("to", &rotate_opt::to)
        .def("forward", &rotate_opt::forward_cuda)
        .def("backward", &rotate_opt::backward_cuda);
};