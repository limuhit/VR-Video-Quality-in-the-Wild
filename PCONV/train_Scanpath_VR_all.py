...
import random
from data_loader import Viewport_all_images
from utils import performance_fit
from utils import plcc_loss, plcc_rank_loss
from model import RN50, RN50_IQA, RN18, RN18_IQA, RN34, RN34_IQA
...
def main(config):
    ...
    for i in range(10):
        ...
        if config.model_name == 'RN18':
            ...
            model = RN18.ResNet18('/data0/wen/resnet/resnet18-f37072fd.pth')
        ...
        if config.database == 'VR_all':
            videos_dir = '.ERP_frame/viewport_224_fp1_spAB/'
            datainfo_train = 'data/scanpathAB_mos_all_20.csv'
            datainfo_val = 'data/scanpathAB_mos_all_20.csv'
            datainfo_test = 'data/scanpathAB_mos_all_20.csv'

            trainset = Viewport_all_images(videos_dir, datainfo_train, transformations_train, 'VR_all_train',
                                           config.crop_size, seed)
            valset = Viewport_all_images(videos_dir, datainfo_val, transformations_train, 'VR_all_val', config.crop_size, seed)
            testset_7A = Viewport_all_images(videos_dir, datainfo_test, transformations_test, 'VR_all_7A_test', config.crop_size, seed)
            testset_7B = Viewport_all_images(videos_dir, datainfo_test, transformations_test, 'VR_all_7B_test', config.crop_size, seed)
            testset_15A = Viewport_all_images(videos_dir, datainfo_test, transformations_test, 'VR_all_15A_test', config.crop_size, seed)
            testset_15B = Viewport_all_images(videos_dir, datainfo_test, transformations_test, 'VR_all_15B_test', config.crop_size, seed)
        ...
        for epoch in range(config.epochs):
            ...
            for i, (video, mos, _) in enumerate(train_loader):
                ...
                labels_mean = torch.mean(labels, 1)
                outputs_mean = torch.mean(outputs, 1)

                optimizer.zero_grad()

                loss = criterion(labels_mean, outputs_mean)
            ...
            with torch.no_grad():
                ...
                for i, (video, mos, video_name) in enumerate(val_loader):
                    ...
                    video_name_list = video_name_list + list(video_name)

                df = pd.DataFrame({'video_name_list': video_name_list,
                                   'label': label,
                                   'y_output': y_output})
                label_mean = df.groupby('video_name_list').mean()['label'].tolist()
                y_output_mean = df.groupby('video_name_list').mean()['y_output'].tolist()
                val_PLCC, val_SRCC, val_KRCC, val_RMSE = performance_fit(label_mean, y_output_mean)
                ...
                y_output_7A = []
                video_name_list_7A = []
                for i, (video, mos, video_name) in enumerate(test_loader_7A):
                    video = video.to(device)
                    tmp_label = mos.tolist()
                    ...
                    video_name_list_7A = video_name_list_7A + list(video_name)

                df = pd.DataFrame({'video_name_list': video_name_list_7A,
                                   'label': label_7A,
                                   'y_output': y_output_7A})
                label_7A_mean = df.groupby('video_name_list').mean()['label'].tolist()
                y_output_7A_mean = df.groupby('video_name_list').mean()['y_output'].tolist()
                test_PLCC_7A, test_SRCC_7A, test_KRCC_7A, test_RMSE_7A = performance_fit(label_7A_mean, y_output_7A_mean)
                ...
                y_output_7B = []
                video_name_list_7B = []
                for i, (video, mos, video_name) in enumerate(test_loader_7B):
                    video = video.to(device)
                    tmp_label = mos.tolist()
                    ...
                    video_name_list_7B = video_name_list_7B + list(video_name)

                df = pd.DataFrame({'video_name_list': video_name_list_7B,
                                   'label': label_7B,
                                   'y_output': y_output_7B})
                label_7B_mean = df.groupby('video_name_list').mean()['label'].tolist()
                y_output_7B_mean = df.groupby('video_name_list').mean()['y_output'].tolist()
                test_PLCC_7B, test_SRCC_7B, test_KRCC_7B, test_RMSE_7B = performance_fit(label_7B_mean, y_output_7B_mean)
                ...
                y_output_15A = []
                video_name_list_15A = []
                for i, (video, mos, video_name) in enumerate(test_loader_15A):
                    video = video.to(device)
                    tmp_label = mos.tolist()
                    ...
                    video_name_list_15A = video_name_list_15A + list(video_name)

                df = pd.DataFrame({'video_name_list': video_name_list_15A,
                                   'label': label_15A,
                                   'y_output': y_output_15A})
                label_15A_mean = df.groupby('video_name_list').mean()['label'].tolist()
                y_output_15A_mean = df.groupby('video_name_list').mean()['y_output'].tolist()
                test_PLCC_15A, test_SRCC_15A, test_KRCC_15A, test_RMSE_15A = performance_fit(label_15A_mean,
                                                                                             y_output_15A_mean)
                ...
                y_output_15B = []
                video_name_list_15B = []
                for i, (video, mos, video_name) in enumerate(test_loader_15B):
                    video = video.to(device)
                    tmp_label = mos.tolist()
                    ...
                    video_name_list_15B = video_name_list_15B + list(video_name)

                df = pd.DataFrame({'video_name_list': video_name_list_15B,
                                   'label': label_15B,
                                   'y_output': y_output_15B})
                label_15B_mean = df.groupby('video_name_list').mean()['label'].tolist()
                y_output_15B_mean = df.groupby('video_name_list').mean()['y_output'].tolist()
                test_PLCC_15B, test_SRCC_15B, test_KRCC_15B, test_RMSE_15B = performance_fit(label_15B_mean,
                                                                                             y_output_15B_mean)
                ...
                label_7AB = label_7A_mean + label_7B_mean
                y_output_7AB = y_output_7A_mean + y_output_7B_mean
                ...
                label_15AB = label_15A_mean + label_15B_mean
                y_output_15AB = y_output_15A_mean + y_output_15B_mean
