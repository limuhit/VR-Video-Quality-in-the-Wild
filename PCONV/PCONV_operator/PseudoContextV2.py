...
class PseudoEntropyContext(BaseOpModule):
    ...
    def __init__(self, npart, context_version=1, opt=True, rt=20, device=0, time_it=False):
        ...
        weight = set_weight(npart, opt)
        self.op = {gid: PCONV.PseudoEntropyContextOp(npart, rt, context_version, weight, gid, time_it) for gid in
                   self.device_list}
...
class PseudoEntropyPad(BaseOpModule):
    ...
    def __init__(self, pad, npart, ctx: PseudoEntropyContext, device=0, time_it=False):
        ...
        self.op = {gid: PCONV.PseudoEntropyPadOp(pad, npart, ctx.get_addr(gid), gid, time_it) for gid in
                   self.device_list}
...
class PseudoFillV2(BaseOpModule):
    ...
    def __init__(self, pad, npart, ctx, fvalue=0, trim=0, device=0, time_it=False):
        ...
        if isinstance(ctx, PseudoContextV2):
            self.op = {gid: PCONV.PseudoFillOp(pad, npart, fvalue, trim, ctx.get_addr(gid), 0, gid, time_it) for gid in
                       self.device_list}
        elif isinstance(ctx, PseudoEntropyContext):
            self.op = {gid: PCONV.PseudoFillOp(pad, npart, fvalue, trim, ctx.get_addr(gid), 1, gid, time_it) for gid in
                       self.device_list}
        else:
            self.op = {gid: PCONV.PseudoFillOp(pad, npart, fvalue, trim, ctx.get_addr(gid), 2, gid, time_it) for gid in
                       self.device_list}
...
class PseudoQUANTV2(BaseOpModule):
    ...
    def __init__(self, channel, bin_num, npart, ctx: PseudoContextV2, check_iters=100, weight_decay=0.9, ntop=1,
                 top_alpha=0.1, device_id=0, time_flag=False):
        super(PseudoQUANTV2, self).__init__(device_id)
        ta = 1. / (bin_num + 1)
        tb = math.log(ta)
        self.weight = nn.Parameter(
            torch.zeros((channel, bin_num), dtype=torch.float32).to('cuda:%d' % self.device_list[0]))
        self.weight.data[:, 0] = ta
        self.weight.data[:, 1:] = tb
        self.count = nn.Parameter(
            torch.zeros((channel, bin_num), dtype=torch.float32).to('cuda:%d' % self.device_list[0]))
        self.op = {gid: PCONV.PseudoQuantOp(channel, bin_num, npart, weight_decay, check_iters, ntop, top_alpha,
                                            ctx.get_addr(gid), gid, time_flag) for gid in self.device_list}
...
class PseudoDQUANT(BaseOpModule):
    ...
    def __init__(self, channel, bin_num, npart, ctx: PseudoContextV2, device_id=0, time_flag=False):
        ...
        self.weight = nn.Parameter(
            torch.zeros((channel, bin_num), dtype=torch.float32).to('cuda:%d' % self.device_list[0]))
        self.op = {gid: PCONV.PseudoDQuantOp(npart, channel, bin_num, ctx.get_addr(gid), gid, time_flag) for gid in
                   self.device_list}
